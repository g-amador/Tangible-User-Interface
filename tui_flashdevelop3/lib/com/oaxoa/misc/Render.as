package com.oaxoa.misc 
{
	...
	public class Render
	{
		...
		public function Render(stage:Stage = null, dimensions:int = 200):void {
			...
			holder.addEventListener(TouchEvent.DOUBLE_TAP, ondoubleclicktap);
			holder.addEventListener(TouchEvent.TOUCH_MOVE, touchonmove);
			holder.addEventListener(TouchEvent.TOUCH_DOWN, ondown);
			holder.addEventListener(TouchEvent.TOUCH_UP, onup);
		}
		...
		private function touchonmove(event:TouchEvent):void {
			if(adding) {
