package touchAll.pong.cl.kirill 
{
	...
	public class PongGame extends MovieClip {
		...
		public function PongGame(stage:Stage = null, color:uint = 0x000000)	{
			...
			_stage.addEventListener(TouchEvent.TOUCH_MOVE, touchPlayerMovement);
			...
			this.addEventListener(TouchEvent.TAP, StartGame);
			this.addEventListener(MouseEvent.RIGHT_CLICK, StopGame);
			this.addEventListener(TouchEvent.DOUBLE_TAP, StopGame);
		...
		private function touchPlayerMovement(e:TouchEvent):void	{
			if (_Playing) {
